...
import com.cy.util.OffsetUtils
...
object Covid19_WZData_Process {
  def main(args: Array[String]): Unit = {
    ...
    val kafkaParams: Map[String, Object] = Map[String, Object](
      "bootstrap.servers" -> "192.168.100.100:9092,192.168.100.110:9092,192.168.100.120:9092", //kafka集群地址
      "group.id" -> "SparkKafka",
  ...
  PRIMARY KEY (`name`)
    ...
    resultDS.foreachRDD(rdd => {
      rdd.foreachPartition(lines => {
        //1.开启连接
        val conn: Connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/bigdata?characterEncoding=UTF-8&useSSL=false", "root", "root")
        //2.编写sql并获取ps
